...
import { addImport } from './properties'
...
export const addHelperMethod = (classObject, interfaceObject, templateName,
                                propertyName, property, prop, proj, api, fileImports) => {
    ...
    if (trueType.includes('Helper'))
        addImport(proj, api, fileImports, property, trueType)
        // fileImports.add(ensureFile(proj, filePath(property.path)), trueType)
    // Class Methods
